    ...
    for entry in trace:
        if isinstance(entry, list):
            trace_entry = {
                ...
                'squawk': entry[8]['squawk'] if isinstance(entry[8], dict) and 'squawk' in entry[8] else None,
                'category': entry[8]['category'] if isinstance(entry[8], dict) and 'category' in entry[8] else None,
            }
...
final_output_file = '/path/to/output/hires_traces.csv' # update output file
